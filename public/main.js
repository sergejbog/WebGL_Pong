...
let ballVelocity;
let ballRadius = 10.0;
let initialBallVelocity = { x: 4.5, y: 4.5 };
...
function setAttribute(buffer, location, target, size, type, stride, offset, divisor = 0) {
...
function main() {
    ...
    const paddleSize = { x: paddleWidth, y: paddleHeight };
    ...
    const middleLineSize = { x: 10, y: 20 };
    const middleLineOffset = [];
    ...
    function animate(time) {
        ...
        */
        ...
        let nextBallOffset = { x: ballOffset.x + ballVelocity.x, y: ballOffset.y + ballVelocity.y };
        ...
        let distance = { x: Math.abs(nextBallOffset.x - paddleOffset[i].x), y: Math.abs(nextBallOffset.y - paddleOffset[i].y) };
